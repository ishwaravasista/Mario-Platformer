...
public class World2 extends World {
    ...
    public World2() {
        setBackground("img/world2background.png");
        generateWorld();
        Mayflower.stopMusic("audio/world1.mp3");
        Mayflower.playMusic("audio/world2.mp3");

        addObject(new Mario(1,scoreTracker.getScore()),0,150);
    ...
    public void makeHill(int length, int height, int bottomPixel, int leftPixel) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < length; j++) {
                if (i == height - 1) {
                    if (j == 0) {
                        addObject(new HillTop("img/Level2/hillblockleft.png"), leftPixel + (j * size), bottomPixel - (i * size));
                    } else if (j == length - 1) {
                        addObject(new HillTop("img/Level2/hillblockright.png"), leftPixel + (j * size), bottomPixel - (i * size));
                    } else {
                        addObject(new HillTop("img/Level2/hillblocktop.png"), leftPixel + (j * size), bottomPixel - (i * size));
                    }
                } else {
                    addObject(new Hill("img/Level2/hillblock.png"), leftPixel + (j * size), bottomPixel - (i * size));
                }
            }
        }
    }
    ...
    public void makeGround(int length, int height, int bottomPixel, int leftPixel) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < length; j++) {
                if (i == height - 1) {
                    addObject(new Grass("img/Level2/grassblock.png"), leftPixel + (j * size), bottomPixel - (i * size));
                } else {
