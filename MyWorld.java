...
public class MyWorld extends World {
    ...
    public MyWorld() {
        setBackground("img/world1background.png");
        generateWorld();
        Mayflower.playMusic("audio/world1.mp3");
        addObject(new Mario(0,scoreTracker.getScore()),0,150);
        showText("Score: 0 Lives: 3", 10, 30, Color.WHITE);
    }
    ...
    public void makeHill(int length, int height, int bottomPixel, int leftPixel) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < length; j++) {
                if (i == height - 1) {
                    if (j == 0) {
                        addObject(new HillTop("img/Level1/hillblockleft.png"), leftPixel + (j * size), bottomPixel - (i * size));
                    } else if (j == length - 1) {
                        addObject(new HillTop("img/Level1/hillblockright.png"), leftPixel + (j * size), bottomPixel - (i * size));
                    } else {
                        addObject(new HillTop("img/Level1/hillblocktop.png"), leftPixel + (j * size), bottomPixel - (i * size));
                    }
                } else {
                    addObject(new Hill("img/Level1/hillblock.png"), leftPixel + (j * size), bottomPixel - (i * size));
                }
            }
        }
    }
    ...
    public void makeGround(int length, int height, int bottomPixel, int leftPixel) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < length; j++) {
                if (i == height - 1) {
                    addObject(new Grass("img/Level1/grassblock.png"), leftPixel + (j * size), bottomPixel - (i * size));
                } else {
